...
- Tests live in dedicated **test modules** under the plugin's `Source/`, split by where they can actually run (which is also the shippability boundary):
  - **Runtime tests** → `Plugins/<Name>/Source/Nexus<Name>Tests/Private/` — a `DeveloperTool` module. Use this when the test touches only runtime/shippable code (the `Nexus<Name>` runtime module + `NexusCore`) **and** can run outside the editor (`N_TEST_CONTEXT_ANYWHERE` / `_CLIENT` / `_SERVER`).
  - **Editor tests** → `Plugins/<Name>/Source/Nexus<Name>EditorTests/Private/` — an `Editor` module. Use this when the test references an editor-module symbol (`Nexus<Name>Editor`, `NexusCoreEditor`, `UnrealEd`, editor subsystems, data validators, …) **or** is declared `N_TEST_CONTEXT_EDITOR`.
  - Decision rule: a **single** editor-module include, or an `N_TEST_CONTEXT_EDITOR` declaration, puts the file in `Nexus<Name>EditorTests`; everything else belongs in `Nexus<Name>Tests`.
  - Why context counts: `DeveloperTool` modules are compiled into **game** targets in Debug and Development (`bBuildDeveloperTools` is only off for Test/Shipping). An `[EditorContext]` test can never execute there, so leaving it in `Nexus<Name>Tests` only adds game-target build time and binary surface.
...
- **Reflected test-helper types** — a `UCLASS`/`UINTERFACE`/`USTRUCT`/`UENUM` used only by tests (e.g. `NTestPooledActor.h`, `NInterfaceMacrosTestTypes.h`) — must **not** be wrapped in a preprocessor guard at all. UHT emits every reflected type into the module's package registration table *unscoped* (`ClassInfo[]` carries a literal `sizeof(YourType)`), so an `#if` around the type deletes the declaration while leaving the generated reference behind. In a Development or Debug **game** build — where the `DeveloperTool` test module still compiles but `WITH_EDITORONLY_DATA` is `0` — that produces a wall of `C2065 undeclared identifier` / `C2070 illegal sizeof operand` errors in `<Type>.gen.cpp` that no editor build will ever reproduce. Guard the *test bodies* with `#if WITH_TESTS`; leave the reflected fixture types bare. (Do not reach for `#if WITH_TESTS` on the type either — same unscoped-registration problem.) Tag any helper shared across modules with the owning test module's API macro (`NEXUS<UPPERCASEPLUGIN>TESTS_API`), not the editor module's, and keep a single definition — a second copy of the same `UCLASS` name collides in the UObject registry.

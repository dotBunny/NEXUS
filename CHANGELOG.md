...
- The world-collision visualizer now wears a wireframe overlay, in the new `World Assembly (User)` > `Color Palette` > `World Collision` > `Stale` color, while the geometry it is drawing is out of date. A stale preview deliberately goes on showing the last baked state rather than blanking — which is right while authoring, but left it looking exactly like a current one, and trusting what it shows is the entire reason to spawn it.
- The World rail's `Cache World Collision` button wears a warning badge while the level's world collision is unbaked or out of date. Staleness is a condition of the level rather than an event, so it is reported on the button that resolves it, for as long as it is true.
- `Color Palette` > `Bones` > `Unverified`, the color a bone is drawn in when there is no baked world collision for the penetration readout to measure it against. It replaces both the valid and the invalid color in that case: an unmeasured bone reads zero penetration, and zero wearing the valid color is a bone claiming to be clear on the strength of a measurement nobody took.
- `World Assembly (User)` > `Notifications` > `World Collision Unavailable` (default on), for turning off the notification raised when a level's world collision is first found unbaked or out of date. The rail badge and the visualizer's stale wireframe still report it.
- World collision now honors the `NWorldCollision_Ignore` tag on individual **components**, not just on actors, so part of an actor can be excluded without excluding the actor. The case this exists for is generated content: a PCG graph writes its whole output onto one container actor, so an actor tag takes all of it — where tagging the spawner's components individually keeps some of what it produced as collision and leaves the rest as dressing. PCG's mesh-spawner descriptors already carry a `Component Tags` list, which can also be driven from an override pin. `World Assembly` project settings gained `World Collisions` > `Component Ignore Tags`, the component-level counterpart of `Actor Ignore Tags`, and both the bake and the live gather it falls back to read the same list.
- `FNRawMeshFactory::FromActorsInBounds` takes an optional `ComponentIgnoreTags` list and skips any primitive carrying one of them. Defaulted to empty, so callers that do not pass one read every primitive exactly as before. It does not narrow the containing-bounds test, which is still made per actor — an actor made entirely of ignored primitives is still walked and simply emits nothing.
- The cell markup tags are now read on individual **components** as well as on actors. `NCell_Ignore` excludes a component from every cell calculation, and `NCell_HullIgnore`, `NCell_BoundsIgnore` and `NCell_VoxelIgnore` exclude it from the convex hull, the playable bounds and the voxel occupancy respectively — the same tags, read at both levels, so a tag placed on a component means what it means on an actor. It is the cell counterpart of the world-collision change above, and exists for the same case: a generator writes its whole result onto one container actor, so an actor tag takes all of it, where tagging the spawner's components individually keeps part of what it produced in the cell's shape and leaves the rest out. One list drives both levels rather than a pair, so a tag an author adds is honored either way without being added twice — which is why each generation setting's `Actor Ignore Tags` now reads `Ignore Tags`. Only the label changed; the property is unchanged on disk and nothing needs re-authoring. Recalculate any `UNCell` whose level already holds a tagged component, since its hull, bounds and voxel data were built without the exclusion.
- `FNActorUtils::GetFilteredComponentsBoundingBox`, the walk behind `GetBuiltComponentsBoundingBox`, taking a component ignore list plus a flag for whether placeholder bounds are rejected. `FNLevelBoundsFilter` gained a matching `ComponentIgnoreTags`, so `FNLevelUtils::DetermineLevelBounds` can drop a tagged primitive from its actor's contribution without dropping the actor — such an actor is still walked, still measured, and is not reported in the ignored-actor list the caller reads back. Both default to measuring every primitive, so a caller passing no list measures exactly what `AActor::GetComponentsBoundingBox` does. `FNActorUtils::HasAnyComponentTag` is the shared predicate behind them.
- The World rail's `World Collision` tag button acts on the selected **components** when any are selected, and on the selected actors otherwise, on the same add-unless-any-already-carries-it rule. Selecting a component in the level editor leaves its owning actor selected too, so the component selection is what wins when both are present.
- The Cell rail's `Cell Collision` tag button follows the World rail's: it acts on the selected **components** when any are selected, and on the selected actors otherwise. Now that the cell calculations read `NCell_Ignore` at either level, an actor-only button was the only way to place the tag where it is most useful.
- A `Clear Generated Proxies` button on the Quick Assembly toolbar cluster, between the build button and the options dropdown, destroying every generated `ANCellProxy` in the level and the Cell Instances loaded from them. Undoing a run previously meant entering the World Assembly edit mode and finding `Clear All Proxies` on the Organ rail, which is a long way round for the other half of the button you just pressed. Level-wide rather than scoped to the targeted organ, since the toolbar has no notion of which runs came before and a clear that left an earlier organ's proxies standing would read as having failed. Greyed out while an operation is running — the build button beside it stays live so it can cancel, but clearing mid-run would strip proxies out from under an operation still spawning them.
- A junction's `Additional Filled Actors` and `Additional Connected Actors` are now driven at author time, so a generated cell shows the same dressing in the editor viewport that it would in game. Applied when the junction registers, which is the point at which a cell spawned from a proxy learns how it resolved — `BeginPlay`, which drives this at runtime, never runs in an editor world. Author time toggles **visibility only**, through `SetIsTemporarilyHiddenInEditor` rather than `SetActorHiddenInGame` (`bHidden` gates rendering only in a game world): collision is deliberately left alone, because the world-collision gather skips primitives whose collision is switched off and driving it here would make a level's baked pool depend on which junctions happened to connect. The state is transient and untransacted, so nothing is dirtied and a regeneration re-applies it. Junctions in a cell opened for authoring are untouched — only cells spawned from a proxy resolve. Fillers and junction connectors are still runtime-only; they are built by the game-only `UNWorldAssemblySubsystem`, which does not exist in an editor world.
- **NEXUS: Texture Packer**, a window under `Tools` > `NEXUS` > `Windows` with two modes. Pack takes up to four textures, assigns each to a channel of one output, and writes it; Unpack takes a packed texture and writes one asset per map it holds. Both work out what they are looking at from asset names: each input's suffix identifies the map it carries, the four together identify a layout, and the layout names the result — three textures called `_AO`, `_R` and `_M` propose `T_<shared name>_ORM` in the folder they came from, and a `_ORM` texture proposes the three back. Anything proposed can be overridden, and stops being re-proposed once it is typed into. Channels carrying the same map are treated as one, so a color map packed across RGB unpacks as one RGB texture rather than three grayscales, and a channel holding a single value throughout is left switched off because it is not a map. Generated assets are created dirty and selected in the Content Browser rather than saved, so a run is reviewable before it is committed to; an existing asset is only overwritten when asked, and is checked out of revision control first.
- Per-channel control on the pack side beyond which texture feeds it: which of the source's channels to read (`Auto` reads red from a grayscale source and the matching channel from anything wider), an `Invert` toggle for the case where what you have is a gloss map and what you want is a roughness one, a constant value for a channel no texture feeds, and a `Raw` toggle that takes a source's stored bytes at face value. Channel values are otherwise read and combined in linear 32-bit float, decoded per texture according to its own `sRGB` flag, so a pack sees the values a material sampler would see. Sources of different sizes are resampled onto the largest rather than refused, and the run reports which were resized; packing a gamma-encoded source at 8 bits warns that linearizing it spreads its dark values wider than 8 bits hold, and points at the 16-bit option. A gamma-encoded output is always written at 8 bits, because the wider source formats are linear-only and writing one while flagging the texture sRGB would label linear values as encoded ones.
- `Texture Packer` editor settings, holding the two tables the window reads. `Channel Roles` maps filename suffixes onto map types, and onto the sRGB flag and compression a texture holding one should get; `Packed Layouts` maps a suffix onto the arrangement of roles a packed texture holds. Both ship filled in — the widely-used conventions (`_ORM`, `_ARM`, `_RMA`, `_MRA`, `_MRAO`, `_ORMH`, and the `_BC` / `_N` / `_R` / `_M` / `_AO` / `_H` / `_S` / `_E` / `_A` roles behind them) are flagged as industry standard and labelled that way in the window's pickers, alongside the less universal ones. A project with its own vocabulary edits the tables rather than the code. Where two rows describe the same arrangement under different names — `ORM` and `ARM` are both occlusion/roughness/metallic — the first row is the spelling proposed, so ordering them is how a project states its preference.
- `Texture Packer` entries on the Content Browser's texture context menu. `Pack` and `Unpack` decide everything from the selection's names and write the result without opening anything, which makes unpacking a folder of packed textures one action rather than one per texture; a texture whose name says nothing is skipped with a warning rather than stopping the batch. The remaining two entries open the window on the selection, for exactly that case.
- `UNTexturePackerLibrary`, the Texture Packer as Blueprint-callable functions — `Pack Textures`, `Unpack Texture`, `Unpack Textures`, and `(Advanced)` variants taking a fully-specified request — for driving it from an editor utility widget or a commandlet. The automatic entry points make the same choices the window proposes, so an unattended run produces what opening each texture in turn would have.
- `NexusToolingEditorTests`, an editor test module for `NexusTooling`. It covers the Texture Packer's naming logic: suffix splitting, role inference, layout detection and its greedy fallback, shared-base-name derivation, and the channel-run grouping the unpack side rests on.
- `UNWorldAssemblySubsystem::GetOperationProgress`, the combined 0..1 progress of the operations in flight, and `UNWorldAssemblySubsystem::GetFailedOperationResult`, which reports whether an operation has finished without success — a required organ left unsatisfied — since the world began play or was last cleared, with the result's title and message. Both answer on clients too: while operations run the server now sends each relay a throttled `Client_OperationProgress` (at most four times a second, and only for a change of at least 1%), and a `Client_OperationResult` ahead of `Client_OperationFinished`. Until now a client only ever heard operation tickets, and an operation's result was lost the moment it finished, since the operation is torn down straight after — so nothing outside the subsystem, a loading screen least of all, could tell a generation that failed from one that succeeded. A cancelled operation is not reported as a failure: cancelling is deliberate.
...
- `UNWorldAssemblySubsystem::IsReady` on a host with a local player — a listen server, or standalone — now waits for the cells around that player exactly as a client does, through its own `ANWorldAssemblyRelay`: spawned, and their levels loaded. It previously asked only whether an operation was in flight, but an operation finishes once its cells are spawned and their levels requested, so a host read as ready while every level was still loading. It also now reads as not ready while organs are queued for assembly, since they only become an operation on the subsystem's next tick. A host without a relay — a dedicated server — is unchanged, and none of this depends on `Support Seamless Travel`: a host's own relay comes from the back-fill at world begin play in every mode. `Get Remaining Status` already read the local relay on a host, and its documentation now says so. **Expect `Is Ready?` to return true later** on a listen server or standalone than before, the `DEMO_NWorldAssembly` samples included.

### Fixed

- A junction's `Additional Filled Actors` and `Additional Connected Actors` are now driven for every junction, whatever it resolved to. Only a junction that ended up connected was reliably having its lists applied: everything else reached them through `Fill()`, which returns before touching them when the junction has no eligible filler and the project sets no `Junction Default Filler`, and which is not called at all for a junction whose requirements are `Allow Empty` or for a `Required` one that finished unconnected. Those junctions left both lists at whatever visibility they were authored with, so actors meant to appear only across a connection stayed visible on an opening that had none. The decision is now made once, up front, from the junction's connection state alone. **Expect actors to disappear** in any level that had come to rely on the wrong behavior, since a list authored on a junction that never resolved was in practice always-on.
- A junction on a client no longer resolves its connection state from assembly data that has not replicated yet. `ANCellLevelInstance`'s data can land after its junctions have registered and begun play, and until it does `GetCellLinkDetails` can only answer with a default that reads as unconnected — which had a connected junction hide its connected actors and spawn a filler into an opening another cell was about to meet. Such a junction now waits, and `UpdateFromAssemblyData` resolves it when the data arrives. `ANCellLevelInstance::HasAssemblyData` reports the distinction.
- A filler entry's `Skip Additional Actors` no longer applies when that filler fails to spawn. It was applied at selection, so a spawn that returned nothing left the junction's filled actors hidden on behalf of a filler that never arrived, and the fall-through to the default filler then re-showed them — meaning the flag also did nothing whenever the default filler ran.
- The `World collision cache is out of date` notification is now raised once, when the cache becomes unavailable, rather than every ten seconds for as long as it stays that way. It was rate-limited on wall-clock time against an eight-second expiry, so a stale level kept a notification on screen for four fifths of the time — and because `FNBoneComponentVisualizer` asked on every viewport redraw, it did so whether or not anything was being edited: selecting a bone in a stale level and walking away was enough. It is now edge-triggered on the transition into an unavailable state, taken down by a bake instead of left to expire, and re-armed by that bake so the next edit that invalidates the cache speaks up again. `FNBoneComponentVisualizer` no longer raises it at all — a per-bone, per-redraw draw call is the wrong place to raise UI from, and the bone's own `Unverified` color reports the same thing to the person actually looking at the bone.
- Baking a level's world collision now refreshes a live collision visualizer. A bake changes no actor, so none of the editor change delegates that drive the refresh could see it, and the visualizer went on drawing the pre-bake merge until some unrelated edit happened to dirty it.
...
- `ANWorldAssemblyRelay::IsReady` — and so `UNWorldAssemblySubsystem::IsReady` on a client — no longer reports ready while a nearby-cells request is still awaiting the server's reply. A request clears the cached list before it is sent, and an empty list passed the loaded-cells check vacuously, so for one round trip after the relay registered, and after every operation finished, a client read as settled with nothing synced. Anything gating on it, such as a loading screen, could let go and then watch the cells arrive. It now reads as not ready until the reply lands.
...
> any `UNCell` whose hull was built before this version, as a cell containing non-colliding meshes will now produce a
> tighter one. Assemblies will also place cells where they previously refused to, since they were avoiding geometry
> nothing occupies.

### Added

- World collision is now baked into the level and read back, instead of being gathered from scratch on every assembly. `FNWorldCollisionBaker` walks each organ, extracts and bakes the geometry it can see, and stores the result; `FNCreateVirtualWorldTask` reads that instead of gathering, falling back to a live gather for any organ whose cache no longer matches. What is skipped is the expensive half — the collision extraction, the pending static-mesh compilation flush it forces, the hull conversion, and the landscape tracing.
- `ANWorldCollisionCacheActor`, holding one `FNWorldCollisionPool` per level. Baked hulls live there once and are addressed by key, so two organs whose volumes overlap — or two unbounded organs, which each cover the whole level — cost one key list each rather than a private copy of the geometry. It is an actor rather than data on `AWorldSettings` so that World Partition gives the pool its own external package; every bake would otherwise check out the persistent level.
- `FNOrganCollisionCache` on `UNOrganComponent`, recording what world collision that organ was baked against as a fingerprint plus a sorted list of keys into the level's pool. Keys rather than indices: a key the pool no longer holds is a detectable miss, where an index would silently resolve to whatever geometry now occupies that slot.
- `FNWorldCollisionFingerprint`, summarizing everything a gather would read — the region asked for, the settings governing which actors qualify, and the identity, placement and collision geometry of every actor that passes. Folded together commutatively, because the order a level is walked in is not stable between the editor that bakes a cache and the game that reads it, and an order-dependent summary would mismatch on every run.
- `Cache World Collision` on the World Assembly edit mode's World rail, baking the selected organs or every organ in the level when none are selected. Always a forced re-bake, since someone reaching for the button has a reason to distrust what is stored. Reports progress per organ and can be cancelled; cancelling leaves the level consistent, because each organ's stored fingerprint still describes whatever it was last baked against.
- A read-only `World Collision Cache` row on `UNOrganComponent`'s details panel, reporting how many elements are cached, when they were baked, and whether they still match the level, with a `Bake` button beside it. The state is fingerprinted live, so it answers whether a run right now could use the cache rather than merely whether one was ever baked.
- `World Assembly` project settings `World Collision Cache` (`FNWorldAssemblyCollisionCacheSettings`): `Cache On Save`, `Use Cache` and `Validate Cache`. Deliberately separate from `World Collisions`, whose every field is hashed into the fingerprint — folding cache policy in there would make toggling one of these invalidate every cache in the project for a setting that changed no geometry.
- `FNRawMeshSource`, an optional per-mesh provenance record from `FNRawMeshFactory::FromActorsInBounds` naming the component, instance and element each emitted mesh came from. Left null by callers that only want the geometry; it exists so a consumer that persists what it gathered can recognise the same element in a later gather.
- `Minimum Floor` and `Maximum Ceiling` on a tissue entry (`FNTissueEntry`) and on `UNOrganComponent`, each behind its own `Use` toggle, keeping a cell out of any placement whose world bounds would reach below the floor or above the ceiling. Both are absolute world-space Z rather than organ-relative, and organ and tissue-entry limits combine by narrowing. Data validation rejects an entry whose floor sits above its own ceiling.
- Junctions the graph builders leave unmatched are now paired up and bridged with geometry. `FNConnectJunctionsTask` matches each open junction to the nearest other cell's open junction of the same socket size within `Maximum Range`, routes a spline between them, and proves the swept volume clear of world geometry, placed cells and connectors already accepted. Paired junctions link, so neither is capped off with a filler.
- `INCellJunctionConnector` actors are spawned for those pairings. `FNSpawnJunctionConnectorsTask` hands each to `UNWorldAssemblySubsystem`, which builds it once both cells report in at `BeginPlay` — time-sliced against `Junction Time Slice`, and rebuilt if either cell streams out and back. `OnConnectJunctions` gained an `FNCellJunctionConnectorPath` parameter carrying the cleared route.
...
- `World Assembly` project settings `Junction Connectors` (`FNWorldAssemblyJunctionConnectorSettings`), mirrored per-operation onto `FNAssemblyOperationSettings`: `Enabled`, `Maximum Range`, `Maximum Spline Length`, `Spline Radius`, `Sample Step`, `Tangent Scale`, `Maximum Avoidance Attempts`, `Avoidance Offset Step`, `Endpoint Exclusion` and `Allow Multiple Cell Connections`. A blocked route retries against a bounded, seed-free set of detours in nearest-first order.
- `Allow Multiple Cell Connections` (default off) caps two cells at one connection between them, so several openings facing each other produce one connector rather than a bundle. Only a direct link blocks. Reported as `Rejected (Existing Connection)`.
- Connector routes are now rejected for turning too tightly. A route folding through itself always fails; above that, `Minimum Turn Radius Scale` sets a navigability floor as a multiple of the socket's half-extent *in the direction of the turn* — `1.0` is the fold point, `2.0` (the default) reads as a corridor-width turn, `0` disables the floor. Too-tight routes retry with longer tangents, bounded by `Maximum Straightening Attempts`. Reported as `Rejected (Turn Radius)`, `Rejected (Folded)` and `Straightening Successes`.
- Candidate junction pairs are now gated on how the two openings are oriented, not just how far apart they are. `FNWorldAssemblyUtils::AreJunctionsWithinConnectionAngles` requires all three of `Maximum Facing Angle`, `Maximum Approach Angle` and `Maximum Elevation Difference` (default 45°, which is what separates a ceiling hatch joined to a wall door from a right-angle corridor bend — both are exactly 90° of facing). Evaluated before any routing, so the pass gets cheaper rather than more expensive. Reported as `Rejected (Angle)`.
- `Connection Constraints` on a junction (`FNCellJunctionConnectionConstraints`) optionally replaces the operation's three angle limits with its own, for any pairing that junction takes part in. Both ends are consulted and the stricter wins, so an override can only narrow what a junction accepts. Distinct from the `Rotation Constraints` beside it, which govern how the owning *cell* may be rotated.
- `Connect Coincidences` (`Assembly|Junction Matching`, default off) mates two unmatched junctions already sitting in the same place facing opposite ways — which a graph looping back on itself, or two organs growing into each other, can otherwise leave capped on both sides of an open doorway. They link as a plain cell mating, so nothing is routed and nothing is spawned. Runs whether or not the connector pass is enabled. Reported as `Inverse Matched`.
- `Disable Connecting` on a junction (`FNCellJunctionDetails::bDisableConnecting`) excludes it from connector pairing entirely. An opted-out junction still mates during graph building, is still eligible for `Connect Coincidences`, and is still filled per its `Requirements`. Reported as `Disabled Junctions`, a subset of `Open Junctions`.
- The World Assembly edit mode now draws accepted connector routes: the center curve in the `Valid` junction color, the four socket-corner curves in a new `Connector Corners` palette entry, and connector-paired junctions tinted distinctly. Works in the default proxy-only preview, where there are no junction components at all. Toggled by `World Assembly (User)` > `Draw Junction Connectors`.
- `FNEvaluateGraphsTask`, a new stage between junction connecting and spawn-list creation, owning hot path resolution, proximity scoring and per-junction link details. All three previously sat inside `FNCreateSpawnsTask`, which is back to flattening graphs into a spawn list. Each pass now completes across every graph before the next begins, rather than interleaving per graph — a connector link can reach a cell in another graph, and interleaving baked in that neighbour's hot path flags before they had been computed. Each pass is timed separately in the operation report, and the stage reports an `Evaluating Graphs` status channel.
- `HotPathShortestScore`, `HotPathSequentialScore` and `ImportanceScore` on `FNCellAssemblyData`, each recording how many cells separate a generated cell from the nearest seed of that kind — `0` on the seed itself, `1` for anything directly connected to one, and so on outward. `ImportanceScore` seeds from the new `NEXUS.WorldAssembly.Flag.Important` tag, which unlike `Hotpath` changes nothing about routing. Distance is counted in cells (bones and null nodes are stepped through for free) and resolved across the whole operation, so a cell one connector from a neighbouring organ's landmark scores `1`. Stored as bytes, saturating at `FNCellAssemblyData::UnreachableScore` (`255`), which also means no seed of that kind is reachable.
- `ANCellLevelInstance` exposes `GetHotPathScore` (the nearer of the two variants), `GetHotPathShortestScore`, `GetHotPathSequentialScore` and `GetImportanceScore`. `UNWorldAssemblyLibrary` mirrors all four for Blueprint widened to `int32`, and adds `Is Near HotPath` / `Is Near Important` threshold predicates with the usual exec-pin twins.
- `FNCellLinkDetails` gained `ConnectedHotPathShortestScore`, `ConnectedHotPathSequentialScore` and `ConnectedImportanceScore`, carrying the proximity scores of the cell on the *far* side of each junction. This is the half a junction cannot work out for itself: its own cell's scores are already reachable, but say nothing about which of that cell's doorways leads inward, and the far cell frequently has not streamed in when `INCellJunctionBeginPlay` fires. Compare against the owning cell to get direction — `ANCellLevelInstance::DoesJunctionLeadTowardHotPath` / `DoesJunctionLeadTowardImportant` and their Blueprint nodes do exactly that, with the usual exec-pin twins. `IsConnectedOnHotPathShortest()` and friends are derived from the score rather than stored, since scoring seeds `0` from exactly the flagged cells; that asks whether the cell *across* the junction is on the route, where `bHotPathShortest` asks whether both are.
...
- `UNTargetPointComponent` now has a presence in the level viewport, drawn by `FNTargetPointComponentVisualizer` as scaled axes plus a world-down reference arrow, as a screen-space handle so it stays visible and clickable at any zoom. Clicking a marker selects the component rather than the actor holding it, so the gizmo lands on the marker being moved and alt-drag duplicates it. Markers are labeled with the component tags Get Component Points filters on, culled past 2500 units except in orthographic views.
...
- `Draw Junction Connector Path` on `UNWorldAssemblyLibrary`, drawing the `FNCellJunctionConnectorPath` handed to a connector through `OnConnectJunctions`. It draws the stored samples, which are the exact points swept for collisions; a `Subdivisions` pin smooths the faceting without moving the line off any tested point.
- `FNRawMeshUtils::MakeConvexPrism` (`NexusCore`): the swept-volume sibling of `MakeBoxHull`, spanning two arbitrary quadrilateral caps with the same production-shaped topology. Cap winding does not matter — a cap wound against the sweep is detected and both are flipped together.
...
- `FNObjectSnapshotUtils::RemoveKnownLeaks` no longer reports objects carrying `RF_WasLoaded` — an asset the engine serialised off disk is loaded, not leaked, and stays resident for the editor session by design. This was failing `UNWorldAssemblyEditorValidator::JunctionMobility`, whose fixture pulls in a junction's editor icon on first registration.
...
- Early experimental support for both Landscape and Mesh Terrains, must be explicitly turned on via World Assembly Settings for world collision, and in the individual cell generation settings to effect them. At times this will break auto calculating on save due to the terrain/mesh terrain not having settled, you have been warned.
- Quick HighRes Screenshot (`FNQuickHighResScreenshot`), bound to `Alt+F9` — one keypress captures a viewport at a preset multiplier through `FViewport::TakeHighResScreenShot` and writes it to the editor's screenshot folder, so what comes out is the rendered view with no editor chrome in it. While a PIE session is up it captures the game viewport, whether play is hosted in the level viewport, a floating window or a standalone one; outside PIE it captures the active level viewport.
- `Tooling (User) > Quick HighRes Screenshot` settings `Enabled` (default off, `ConfigRestartRequired`) and `Multiplier` (default 2.0, clamped 0.5–5.0), held per developer so they survive restarts. `FNToolingEditorBindings` registers the chord under Editor Preferences > Keyboard Shortcuts as `NEXUS: Tooling`, mapped onto both the level editor and play-world command lists so it fires in and out of play. No Tools menu entry — the point of it is the keypress.
- Per-actor selection lock (`FNSelectionLock`), keeping an actor from being picked by clicking it in a level viewport. A `Selection Lock` column sits beside the visibility gutter in the Outliner, and `Lock Selection` is added to the actor context menu, so it appears on an Outliner row and on a viewport right-click alike. Clicking the toggle on a row that is part of the current selection applies to the whole selection. Locked actors stay reachable from the Outliner and by marquee, but draw no selection outline and do not highlight on hover.
- The lock persists as editor-only package meta-data (`NexusSelectionLocked`), so it is stripped at cook, invisible in the Details panel, and travels with the level through source control. It is re-applied whenever something rebuilds the components it lives on — including construction-script reruns, which `bSelectable` cannot survive on its own. Gated by the `Selection Lock` project setting (`Tooling > Selection Lock > Enabled`, default on, `ConfigRestartRequired`); turning it off hides the feature without unlocking anything. Nanite meshes cannot be locked against clicking, as Nanite resolves hit proxies through its own pass.
- A `Display` group on the World Assembly edit mode's Cell rail, with `Draw Bounds`, `Draw Hull` and `Draw Fill Bounds` switching off the red bounds box, the blue collision hull, and the grey box previewing the volume a junction's filler would occupy. They are per-user preferences in `NexusUserSettings.ini` rather than data on the cell, mirrored in `World Assembly (User)` > `Debug`, and gate the component visualizers as well as the edit mode's own render.

### Changed

- The collision visualizer and the bone penetration readout now read the level's baked collision rather than gathering the world themselves, so both show the geometry an assembly will actually collide against instead of a second derivation of it. A level that has never been baked has nothing to show and says so, offering a bake, rather than presenting an empty result that reads as "there is no collision here". A cache that has gone stale keeps drawing the last baked state alongside the notice — every world-geometry edit invalidates it, and blanking on each one would leave the tools unusable while authoring. Editing a bone invalidates nothing, since bones carry no collision and never enter a bake.
- Baking, whether from the rail or on save, is the single writer of world collision, and everything that consumes it now routes through two shared entry points: `FNWorldCollisionBaker::BakeElement` for what the geometry *is*, and `FNWorldCollisionBaker::ResolvePooled` for what a cache can *answer*. Both previously existed in more than one copy, which is how the visualizer came to draw geometry no collision test honored.
- Saving a level bakes any organ whose world collision has changed since its last bake. An organ still matching its fingerprint is skipped, and a level nothing collision-relevant has been touched in skips even the fingerprint pass, so an untouched level costs nothing to save.
- The World Assembly edit mode's rail and panel are hidden for the duration of a play session. Both are overlays on the level viewport, which is what an in-viewport PIE session draws into, so they previously sat over the running game.
- `FNWorldCollisionBaker::BakeOrgans` reports progress through a slow task and can be cancelled from it, one frame per organ plus one for landscape. A bake is synchronous game-thread work — a re-gather flushes pending static-mesh compilation, and sampling landscape traces the physics scene once per sample — so on a large level it is seconds to minutes with the editor otherwise stopped and no indication why.
...
- A primitive with its collision switched off is no longer read as world geometry. The only collision test was actor-wide (`AActor::GetActorEnableCollision`), so an actor that collided through any one component had every one of its primitives extracted — collision on the source asset was enough, whatever the component was set to. The arrangement that exposes it is a container actor holding PCG-generated instances set to `No Collision` beside a separately baked collider: both were emitted, so the geometry appeared twice and assemblies avoided space nothing occupies. Placements that previously failed near such geometry will now succeed, and a `UNCell` recalculated after this change will produce a tighter hull than one baked before it.
- The collision visualizer and the bone penetration readout drew the source collision geometry rather than the convex hulls every collision test in an assembly actually runs against. A doorway arch or a hollow pipe showed its opening while an assembly treated the whole volume as solid — the visualizer showing open space exactly where the assembly sees none, and the bone readout reporting clear for a bone sitting in a concave pocket. Both now bake each element the way an assembly does before merging it.
- `FNRawMeshUtils::CombineMesh` no longer recomputes the centre and bounds of everything merged so far on each call. It ended in a full `CalculateCenterAndBounds`, which walks the whole accumulated vertex buffer — fine for one merge and quadratic for a loop of them, which is what merging a level's collision is. Measured at roughly a hundred million vector adds and a twelve-second editor stall on toggling the collision visualizer. Centre and bounds are now accumulated over the vertices being appended, in the pass that appends them; a mesh whose vertices were populated by hand, and whose bounds were therefore never computed, still falls back to the full walk rather than building on a stale value.
- The divider before the Quick Assembly cluster no longer disappears on entering the World Assembly edit mode. It was keyed to the same predicate as the `Switch To WorldAssembly Editor Mode` button beside it, so it left with that button and abutted Quick Assembly against whatever the level editor put before it. It now stands while either of them does.
- The cell filter the organ context runs to validate itself now states the bone as its attach point. It was the one caller of four leaving `WorldPosition` unset, so the position-sensitive gates read an uninitialized vector there — meaning whether an organ carrying direction-constrained cells validated at all came down to whatever happened to be on the stack.
...
- `FNWorldCollisionCache` and the asynchronous pipeline behind it — the debounced ticker, the per-world rebuild state machine, the cooperative cancel flags and the published-result plumbing. All of it existed to keep a constantly changing source off the draw thread; the collision visualizer and the bone penetration readout now read the level's baked pool, which does not change while you work, so a memo rebuilt on edit is enough. Replaced by `FNWorldCollisionPreview`, at roughly a third of the size.
- `FNWorldAssemblyEditorUtils::RefreshWorldCollisionVisualizerActor` lost its `Bounds` and `OutSourceActors` parameters along with the gather they served. The pool is scoped by the organs referencing it, which is what the caller expressed by passing no bounds, and there is no longer a set of source actors for a caller to test later changes against.

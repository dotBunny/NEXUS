...
- `Minimum Floor` and `Maximum Ceiling` on a tissue entry (`FNTissueEntry`), each behind its own `Use` toggle, keeping a cell out of any placement whose world bounds would reach below the floor or above the ceiling. Both are absolute world-space Z in world units rather than organ-relative, so a tissue authored against one altitude behaves differently when reused by an organ sitting at another. They gate on the rotation-baked world AABB the candidate would occupy — the same box every other placement test compares — so it is the cell's extents that are judged and not its pivot: a cell whose pivot clears a floor of 0 while its underside reaches -50 is rejected for that underside, and one resting flush on the floor is allowed.
- The same four settings on `UNOrganComponent`, applying to every cell that organ places. Independent of `Unbound`, and the only vertical limit an unbound organ can express at all, since the bounds containment its cells would otherwise be clipped by is skipped for it. Organ and tissue-entry limits combine by narrowing — the higher floor and the lower ceiling are the ones enforced — so a cell can be stricter than the organ it is placed in but can never author its way outside it.
- The gate runs during cell filtering rather than at attachment, so a junction that cannot take a height-restricted cell is offered the rest of the pool instead of being left open. That is exact rather than approximate: the required rotation is already composed there for the rotation constraints, so where the candidate would land is derivable without building a node, and it resolves against the same authored bounds through the same corner enumeration the placed node caches — now `FNAssemblyGraphCellNode::ComputeWorldBounds`, shared by both rather than written twice. It costs nothing when neither the organ nor any cell in the pool declares a limit.
- A start cell composes its placement unflipped, matching the bone's facing where every other placement opposes the source junction's. The two resolve a candidate to different places whenever the source carries any pitch or roll, so the height gate composes each the way its own placement path will rather than sharing one.
- Data validation rejects a tissue entry whose floor sits above its own ceiling, and generation warns for the same on an entry or an organ. The window is empty either way, so the cell would otherwise silently never be placed rather than failing anywhere visible.
...
- A `Display` group on the World Assembly edit mode's Cell rail, with `Draw Bounds`, `Draw Hull` and `Draw Fill Bounds` switching off the red bounds box, the blue collision hull, and the grey box extruded from each junction socket that previews the volume its filler would occupy. They are per-user preferences in `NexusUserSettings.ini` rather than data on the cell — which is why they sit apart from the Quick Options above them — and are mirrored in `World Assembly (User)` > `Debug`. All three gate the component visualizers as well as the edit mode's own render, so switching one off holds whether or not the mode is open, and the interactive tools' handles are drawn by the running tool rather than the overlay, so a tool stays usable with its geometry hidden. `Draw Fill Bounds` replaces a hardcoded `bDrawFillDepth` on `UNCellJunctionComponent::DrawDebugPDI`, which now takes it as a parameter.

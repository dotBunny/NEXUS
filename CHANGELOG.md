...
- `UNTargetPointComponent` now has a presence in the level viewport, drawn by `FNTargetPointComponentVisualizer`. Get Target Points emits the component transform whole, so an unnoticed rotation or scale reaches every spawner downstream — a marker is drawn as scaled axes, alongside an arrow pointing world down in the Get Target Points node's own color: a fixed reference against the axes beside it, which are the ones that turn with the marker, and a tie between what is in the viewport and the node that consumes it. The point itself is a screen-space handle rather than world-space geometry, so it stays both visible and clickable at any zoom, where a wire shape sized in world units shrinks to a speck exactly when a set of markers is being surveyed from a distance.
...
- `Tooling (User) > Quick HighRes Screenshot > Multiplier` (default 2.0) holding that multiplier per developer, so it survives restarts. The 1.0–10.0 range mirrors the engine's own limits, and a multiplier that would exceed the GPU's maximum texture dimension is still rejected by the engine's existing warning rather than crashing.

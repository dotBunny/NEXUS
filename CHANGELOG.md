...
- Junction pairs whose sockets open *away* from each other are now rejected. Nothing gates candidate pairs on facing, so such a pair still reaches the router, but meeting both sockets square on forces the route to double back out of each opening — and no tangent scale opens that turn wide enough to clear the socket. The fold check is what catches it, so these no longer produce unbuildable connectors.

...
- The lock persists as editor-only package meta-data on the actor (`NexusSelectionLocked`), so it is stripped at cook, invisible in the Details panel, and travels with the level through source control. It is read back when a map opens and re-applied to actors that spawn, stream in or are reinstanced by a Blueprint recompile. Duplicating a locked actor produces an unlocked one, and the toggle is not undoable — package meta-data is not transactional.

...
public:
	...
	/** Hold Space then left-drag to pan the foreground graph. */
	...
	/** Multiplier applied to drag distance when using Space-to-pan. */
	...
	/** Editor frame-rate cap (FPS); 0 uses the engine default. */
	...
	/** Keep the editor's Show Frame Rate & Memory option enabled even if local settings are reset. */
	...
	/** Duration, in seconds, the leak check runs. */
	...
	/** Color of a distribution visualizer's inner boundary. */
	...
	/** Thickness of drawn debug lines. */
	...
	/** Number of line segments used to draw a debug circle. */
	...
	/** Clear the (unused) log files before running a multiplayer test. */
	...
	/** Number of clients to spawn for a multiplayer test. */
	...
	/** Window size for each spawned client. */
	...
	/** Capture a per-client network profile (.nprof) under Saved/Profiling. */
	...
	/** Extra command-line parameters passed to each launched client. */
	...
	/** Force clients to receive network packets out of order. */
	...
	/** Launch a separate server even when the net mode would not otherwise require one (rarely needed). */

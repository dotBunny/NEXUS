...
/** Size the report link's external-editor icon draws at, against the 8-point label it follows. */
static constexpr float ReportLinkIconSize = 12.0f;
...
void SNWorldAssemblyOperations::Construct(const FArguments& InArgs)
{
	ChildSlot
	[
	...
	OperationStateChangedHandle = FNWorldAssemblyRegistry::OnOperationStateChanged.AddSP(
		this, &SNWorldAssemblyOperations::OnOperationStateChanged);

	OperationChannelsChangedHandle = FNWorldAssemblyRegistry::OnOperationChannelsChanged.AddSP(
		this, &SNWorldAssemblyOperations::OnOperationChannelsChanged);
}

SNWorldAssemblyOperations::~SNWorldAssemblyOperations()
{
...
void SNWorldAssemblyOperations::OnOperationStateChanged(UNAssemblyOperation* Operation, const ENWorldAssemblyOperationState NewState)
{
...
void SNWorldAssemblyOperations::OnOperationChannelsChanged(UNAssemblyOperation* Operation, const TArray<FNStatusChannelUpdate>& Changes)
{
	if (Operation == nullptr || !ShouldShowOperation(Operation)) return;
...
void SNWorldAssemblyOperations::Rebuild()
{
...
TSharedRef<SWidget> SNWorldAssemblyOperations::CreateIdleBlock()
{
	...
	return SNew(SVerticalBox)
		...
		// so a filled slot underlines the empty space past the label as well as the label.
		.HAlign(HAlign_Left)
		.Padding(16.0f, 2.0f, 0.0f, 0.0f)
		[
			// Visibility and tooltip sit on the row rather than on the link, so the icon collapses with it rather than
			// being left behind pointing at a report that is not there, and so hovering either one reports the path.
			SNew(SHorizontalBox)
			.ToolTipText_Lambda([]()
			{
				const FNAssemblyRunSummary* Summary = FindLastRunSummary();
				return Summary != nullptr && !Summary->ReportFilePath.IsEmpty()
					? FText::FromString(FPaths::ConvertRelativePathToFull(Summary->ReportFilePath))
					: FText::GetEmpty();
			})
			.Visibility_Lambda([]()
			{
			...
			[
				...
				// Named for what it does rather than for the file it opens. The generated filename is long, carries a
				// timestamp, and is the same shape every run, so it read as noise in a panel this narrow while saying
				// nothing the line above has not already said about which run this is.
				.Text(LOCTEXT("Operations_LastRunReport", "Open Report"))
				.OnNavigate_Lambda([]()
				{
					const FNAssemblyRunSummary* Summary = FindLastRunSummary();
			...
			[
				// Sized down from the brush's own 16, which would stand well clear of an 8-point label, and colored
				// from the link's text style so the two read as one thing at rest.
				SNew(SImage)
				.Image(FAppStyle::Get().GetBrush("Icons.OpenInExternalEditor"))
				.DesiredSizeOverride(FVector2D(ReportLinkIconSize, ReportLinkIconSize))
				.ColorAndOpacity(GetReportLinkTextStyle().ColorAndOpacity)
			]
		];
}

TSharedRef<SWidget> SNWorldAssemblyOperations::CreateOperationBlock(UNAssemblyOperation* Operation) const
{
...
TSharedRef<SWidget> SNWorldAssemblyOperations::CreateChannelBar(const int32 Ticket, const int32 ChannelId) const
{
...
bool SNWorldAssemblyOperations::ShouldShowOperation(const UNAssemblyOperation* Operation)
{

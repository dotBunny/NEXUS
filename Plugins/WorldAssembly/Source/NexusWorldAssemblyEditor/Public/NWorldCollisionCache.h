 ...
 * uses (the simple collision of every actor passing the world-actor filter, plus the sampled surface of any landscape
 * among them, merged into one mesh).
 *
...
public:
	 ...
	 * overlaps Bounds into one world-space mesh, along with the sampled surface of any landscape among them. This is the
	 * shared "what counts as world geometry" definition; Get memoizes it. Exposed (and unit-tested) directly so the
	 * build can be exercised without the cache's static state.
	 ...
	 *        Also narrows the area any landscape is sampled over.
	 * @param OutSourceActors Optional; receives the filtered world actors the mesh was gathered from.
	 * @note Game-thread only (walks live actors, traces the physics scene, and flushes async static-mesh compilation).
	 * @note Cost scales with landscape size and `Landscape Sample Spacing`, since a landscape contributes one downward
	 *       trace per sample rather than geometry that is simply read. Setting that spacing to 0 leaves landscape out.
	 */

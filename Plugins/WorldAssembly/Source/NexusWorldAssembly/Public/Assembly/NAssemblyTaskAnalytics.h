...
/** Member name used by every N_ASSEMBLY_ANALYTICS_* macro for the analytics shared pointer. */
#define N_ASSEMBLY_ANALYTICS_MEMBER_PTR AnalyticsPtr
/** Member name used by index-based macros for the per-task analytics record index. */
#define N_ASSEMBLY_ANALYTICS_MEMBER_INDEX AnalyticsIndex
...
#define N_ASSEMBLY_ANALYTICS_CREATE N_ASSEMBLY_ANALYTICS_MEMBER_PTR = MakeShared<FNAssemblyTaskAnalytics, ESPMode::ThreadSafe>(Operation->GetDisplayName());
/** Append the analytics shared pointer to a constructor parameter list. */
#define N_ASSEMBLY_ANALYTICS_CONSTRUCTOR , const TSharedPtr<FNAssemblyTaskAnalytics>& N_ASSEMBLY_ANALYTICS_MEMBER_PTR
/** Append the analytics initializer to a constructor's member initializer list. */
#define N_ASSEMBLY_ANALYTICS_INITIALIZER , N_ASSEMBLY_ANALYTICS_MEMBER_PTR(N_ASSEMBLY_ANALYTICS_MEMBER_PTR.ToSharedRef())
/** Append the analytics shared pointer when forwarding it through an existing argument list. */
#define N_ASSEMBLY_ANALYTICS_CLASS_REF , N_ASSEMBLY_ANALYTICS_MEMBER_PTR
/** Declare the analytics shared pointer as a class member. */
#define N_ASSEMBLY_ANALYTICS_SHARED_PTR	TSharedPtr<FNAssemblyTaskAnalytics> N_ASSEMBLY_ANALYTICS_MEMBER_PTR;
/** Declare the analytics shared reference as a class member. */
#define N_ASSEMBLY_ANALYTICS_SHARED_REF TSharedRef<FNAssemblyTaskAnalytics> N_ASSEMBLY_ANALYTICS_MEMBER_PTR;
/** Reset the analytics shared pointer once the owner is finished with it. */
#define N_ASSEMBLY_ANALYTICS_DELETE N_ASSEMBLY_ANALYTICS_MEMBER_PTR.Reset();
/** Declare a local index used to address the per-task analytics record on the analytics object. */
#define N_ASSEMBLY_ANALYTICS_INDEX_LOCAL int32 N_ASSEMBLY_ANALYTICS_MEMBER_INDEX = -1;

/** Forward a no-argument analytics call. */
#define N_ASSEMBLY_ANALYTICS(Method) \
	N_ASSEMBLY_ANALYTICS_MEMBER_PTR->Method();
/** Forward a one-argument analytics call. */
#define N_ASSEMBLY_ANALYTICS_ONE_PARAM(Method, Parameter) \
	N_ASSEMBLY_ANALYTICS_MEMBER_PTR->Method(Parameter);
/** Forward a two-argument analytics call. */
#define N_ASSEMBLY_ANALYTICS_TWO_PARAM(Method, Parameter1, Parameter2) \
	N_ASSEMBLY_ANALYTICS_MEMBER_PTR->Method(Parameter1, Parameter2);
/** Forward a three-argument analytics call. */
#define N_ASSEMBLY_ANALYTICS_THREE_PARAM(Method, Parameter1, Parameter2, Parameter3) \
	N_ASSEMBLY_ANALYTICS_MEMBER_PTR->Method(Parameter1, Parameter2, Parameter3);
/** Forward a four-argument analytics call. */
#define N_ASSEMBLY_ANALYTICS_FOUR_PARAM(Method, Parameter1, Parameter2, Parameter3, Parameter4) \
	N_ASSEMBLY_ANALYTICS_MEMBER_PTR->Method(Parameter1, Parameter2, Parameter3, Parameter4);
/** Forward a five-argument analytics call. */
#define N_ASSEMBLY_ANALYTICS_FIVE_PARAM(Method, Parameter1, Parameter2, Parameter3, Parameter4, Parameter5) \
	N_ASSEMBLY_ANALYTICS_MEMBER_PTR->Method(Parameter1, Parameter2, Parameter3, Parameter4, Parameter5);
/** Forward an analytics call that takes the previously-stashed analytics record index. */
#define N_ASSEMBLY_ANALYTICS_INDEX(Method) \
	N_ASSEMBLY_ANALYTICS_MEMBER_PTR->Method(N_ASSEMBLY_ANALYTICS_MEMBER_INDEX);
/** Forward an analytics call that returns a record index and stash the result into the existing index member. */
#define N_ASSEMBLY_ANALYTICS_INDEX_SET(Method) \
	N_ASSEMBLY_ANALYTICS_MEMBER_INDEX = N_ASSEMBLY_ANALYTICS_MEMBER_PTR->Method();
/** Define a new const local index initialized from an analytics call that returns a record index. */
#define N_ASSEMBLY_ANALYTICS_INDEX_DEFINE(Method) \
	const int32 N_ASSEMBLY_ANALYTICS_MEMBER_INDEX = N_ASSEMBLY_ANALYTICS_MEMBER_PTR->Method();
#else
#define N_ASSEMBLY_ANALYTICS_CREATE
#define N_ASSEMBLY_ANALYTICS_CONSTRUCTOR
#define N_ASSEMBLY_ANALYTICS_INITIALIZER
#define N_ASSEMBLY_ANALYTICS_CLASS_REF
#define N_ASSEMBLY_ANALYTICS_SHARED_PTR
#define N_ASSEMBLY_ANALYTICS_SHARED_REF
#define N_ASSEMBLY_ANALYTICS_DELETE
#define N_ASSEMBLY_ANALYTICS_INDEX_LOCAL
...
/**
 * Aggregates timing and counter analytics for one UNAssemblyOperation's full task-graph build.
 *
 ...
 * should be held as TSharedPtr<FNAssemblyTaskAnalytics, ESPMode::ThreadSafe>.
 */
class FNAssemblyTaskAnalytics
{
public:
	explicit FNAssemblyTaskAnalytics(const FText& DisplayName) : DisplayName(DisplayName) {};
...
private:
	...
	FNWorldAssemblyTaskTimer TaskGraphCreationTimer = FNWorldAssemblyTaskTimer();

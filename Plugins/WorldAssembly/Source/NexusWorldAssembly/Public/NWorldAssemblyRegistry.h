 ...
 * @see <a href="https://nexus-framework.com/docs/world-assembly/types/world-assembly-registry/">FNWorldAssemblyRegistry</a>
 */
...
public:
	...
	static bool RegisterOperation(UNAssemblyOperation* Operation);
	...
	static bool UnregisterOperation(UNAssemblyOperation* Operation);
...
private:
	...
	static void NotifyOfStateChange(UNAssemblyOperation* Operation, ENWorldAssemblyOperationState NewState)
	{
		// We don't have a display name at registration, so the log for registration is done manually
		UE_CLOG(NewState != ENWorldAssemblyOperationState::Registered, LogNexusWorldAssembly, Log, TEXT("[%s] Changed State(%s)"), *Operation->GetDisplayName().ToString(), *UNAssemblyOperation::GetStringFromState(NewState));

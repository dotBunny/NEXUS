...
void UNDeveloperSubsystem::Tick(float DeltaTime)
{
	...
	{
		N_LOG(Error, TEXT("[NDeveloperSubsystem::Tick] Object count SNAPSHOT threshold met with %d objects."), ObjectCount);
		...
		if (bShouldOutputSnapshot)
		{
			...
			N_LOG(Error, TEXT("[NDeveloperSubsystem::Tick] SNAPSHOT written to %s."), *DumpFilePath);
		}
	...
	if (ObjectCount >= ObjectCountCompareThreshold && !bPassedObjectCountCompareThreshold)
	{
		...
		N_LOG(Error, TEXT("[NDeveloperSubsystem::Tick] Object count COMPARE threshold met with %d objects."), ObjectCount);
		...
		N_LOG(Error, TEXT("[NDeveloperSubsystem::Tick] COMPARE written to %s."), *DumpFilePath);

...
{
	FScopedSlowTask MainTask = FScopedSlowTask(4, NSLOCTEXT("NexusToolsEditor", "FindAndFix_PoseAssets_OutOfDateAnimationSource", "Find & Fix: Outdated PoseAssets Animation Sources"));
	...
	if (bIsContextMenu)
	{
		MainTask.EnterProgressFrame(1, NSLOCTEXT("NexusToolsEditor", "FindAndFix_PoseAssets_OutOfDateAnimationSource_Step1", "Collecting ..."));
	...
	{
		MainTask.EnterProgressFrame(1, NSLOCTEXT("NexusToolsEditor", "FindAndFix_PoseAssets_OutOfDateAnimationSource_Step1", "Collecting Everything ..."));
		SelectedPaths = { TEXT("/Game") };
	}
	const int PathCount = SelectedPaths.Num();

	// STEP 2 - Look for PoseAssets
	MainTask.EnterProgressFrame(1, NSLOCTEXT("NexusToolsEditor", "FindAndFix_PoseAssets_OutOfDateAnimationSource_Step2", "Scanning ..."));
	FScopedSlowTask ScanTask = FScopedSlowTask(PathCount, NSLOCTEXT("NexusToolsEditor", "FindAndFix_PoseAssets_OutOfDateAnimationSource_Step2_Scan", "Scanning For Pose Assets"));
	ScanTask.MakeDialog(false);
	for (int i = 0; i < PathCount; i++)
	{
		SelectedPaths[i].RemoveFromStart(TEXT("/All"));
		ScanTask.EnterProgressFrame(1, FText::Format(NSLOCTEXT("NexusToolsEditor", "FindAndFix_PoseAssets_OutOfDateAnimationSource_Step2_Item", "Scanning {0}"), FText::FromString(SelectedPaths[i])));
	...
	MainTask.EnterProgressFrame(1, NSLOCTEXT("NexusToolsEditor", "FindAndFix_PoseAssets_OutOfDateAnimationSource_Step3", "Processing ..."));
	FScopedSlowTask FixTask = FScopedSlowTask(PoseAssets.Num(), NSLOCTEXT("NexusToolsEditor", "OutOfDateAnimationSource_Fix", "Processing Pose Assets"));
	...
	for (int i = 0; i < PoseAssets.Num(); i++)
	{
		...
		FixTask.EnterProgressFrame(1, FText::Format(NSLOCTEXT("NexusToolsEditor", "OutOfDateAnimationSource_Fix_Item", "Processing {0}"), FText::FromString(PoseAssets[i]->GetName())));

		UE_LOG(LogNexusToolsEditor, Log, TEXT("Updating out-of-date UPose(%s)."), *PoseAsset->GetName());
	...
	UE_LOG(LogNexusToolsEditor, Log, TEXT("Fixed %i out-of-date UPose assets."), ProblemsFixed);

	// Step 4 - Cleanup
	MainTask.EnterProgressFrame(1, NSLOCTEXT("NexusToolsEditor", "FindAndFix_PoseAssets_OutOfDateAnimationSource_Step4", "Cleanup ..."));
